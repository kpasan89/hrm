...
import lk.gov.sp.healthdept.entity.Item;
...
@ManagedBean
@SessionScoped
public final class ItemController  implements Serializable {
    ...
    private DataModel<Item> items = null;
    ...
    public List<Item> getLstItems() {
        return getFacade().findBySQL("Select d From Item d");
    }
    ...
    public Item searchItem(String itemName, boolean createNewIfNotPresent) {
        Item searchedItem = null;
        items = new ListDataModel(getFacade().findAll("name", itemName, true));
        if (items.getRowCount() > 0) {
            items.setRowIndex(0);
            searchedItem = (Item) items.getRowData();
