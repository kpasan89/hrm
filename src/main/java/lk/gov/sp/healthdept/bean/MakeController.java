...
@ManagedBean
@SessionScoped
public final class MakeController  implements Serializable {
    ...
    List<Make> lstItems;
    private Make current;
    private DataModel<Make> items = null;
    ...
    public List<Make> getLstItems() {
        return getFacade().findBySQL("Select d From Make d");
    }

    public void setLstItems(List<Make> lstItems) {
    ...
    public void setCurrent(Make current) {
    ...
    public DataModel<Make> getItems() {
        items = new ListDataModel(getFacade().findBySQL("SELECT m FROM Make m WHERE m.retired=false ORDER BY m.name"));
        return items;
    }
    ...
    public Make searchItem(String itemName, boolean createNewIfNotPresent) {
        Make searchedItem = null;
        items = new ListDataModel(getFacade().findAll("name", itemName, true));
        if (items.getRowCount() > 0) {
            items.setRowIndex(0);
            searchedItem = (Make) items.getRowData();
        } else if (createNewIfNotPresent) {
            searchedItem = new Make();
        ...
        public String getAsString(FacesContext facesContext, UIComponent component, Object object) {
            ...
            if (object instanceof Make) {
                ...
                throw new IllegalArgumentException("object " + object + " is of type "
                        + object.getClass().getName() + "; expected type: " + MakeController.class.getName());
            }
        }
    }
}
