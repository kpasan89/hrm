...
import lk.gov.sp.healthdept.facade.MedicineGroupFacade;
import lk.gov.sp.healthdept.entity.MedicineGroup;
...
@ManagedBean
@SessionScoped
public final class MedicineGroupController  implements Serializable {

    @EJB
    private MedicineGroupFacade ejbFacade;
    SessionController sessionController = new SessionController();
    List<MedicineGroup> lstItems;
    private MedicineGroup current;
    private DataModel<MedicineGroup> items = null;
    private int selectedItemIndex;
    boolean selectControlDisable = false;
    boolean modifyControlDisable = true;
    String selectText = "";

    public MedicineGroupController() {
    }

    public List<MedicineGroup> getLstItems() {
        return getFacade().findBySQL("Select d From MedicineGroup d");
    }

    public void setLstItems(List<MedicineGroup> lstItems) {
    ...
    public MedicineGroup getCurrent() {
        if (current == null) {
            current = new MedicineGroup();
        }
        return current;
    }

    public void setCurrent(MedicineGroup current) {
    ...
    public DataModel<MedicineGroup> getItems() {
        ...
        if (items == null) {
            if (selectText.equals("")) {
                ...
                if (items.getRowCount() > 0) {
                    items.setRowIndex(0);
                    current = (MedicineGroup) items.getRowData();
    ...
    public MedicineGroup searchItem(String itemName, boolean createNewIfNotPresent) {
        MedicineGroup searchedItem = null;
        items = new ListDataModel(getFacade().findAll("name", itemName, true));
        if (items.getRowCount() > 0) {
            items.setRowIndex(0);
            searchedItem = (MedicineGroup) items.getRowData();
        } else if (createNewIfNotPresent) {
            searchedItem = new MedicineGroup();
