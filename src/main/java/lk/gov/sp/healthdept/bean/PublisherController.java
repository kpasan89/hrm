...
import lk.gov.sp.healthdept.facade.PublisherFacade;
import lk.gov.sp.healthdept.entity.Publisher;
...
@ManagedBean
@SessionScoped
public final class PublisherController  implements Serializable {

    @EJB
    private PublisherFacade ejbFacade;
    @ManagedProperty(value = "#{sessionController}")
    SessionController sessionController;
    List<Publisher> lstItems;
    private Publisher current;
    private DataModel<Publisher> items = null;
    ...
    public List<Publisher> getLstItems() {
        return getFacade().findBySQL("Select d From Publisher d");
    }

    public void setLstItems(List<Publisher> lstItems) {
    ...
    public Publisher getCurrent() {
        if (current == null) {
            current = new Publisher();
        }
        return current;
    }

    public void setCurrent(Publisher current) {
    ...
    public DataModel<Publisher> getItems() {
    ...
    public PublisherFacade getEjbFacade() {
        return ejbFacade;
    }

    public void setEjbFacade(PublisherFacade ejbFacade) {
        ...
        if (items == null) {
            if (selectText.equals("")) {
                ...
                if (items.getRowCount() > 0) {
                    items.setRowIndex(0);
                    current = (Publisher) items.getRowData();
    ...
    public Publisher searchItem(String itemName, boolean createNewIfNotPresent) {
        Publisher searchedItem = null;
        items = new ListDataModel(getFacade().findAll("name", itemName, true));
        if (items.getRowCount() > 0) {
            items.setRowIndex(0);
            searchedItem = (Publisher) items.getRowData();
        } else if (createNewIfNotPresent) {
            searchedItem = new Publisher();
        ...
        public String getAsString(FacesContext facesContext, UIComponent component, Object object) {
            ...
            if (object instanceof Publisher) {
                ...
                throw new IllegalArgumentException("object " + object + " is of type "
                        + object.getClass().getName() + "; expected type: " + PublisherController.class.getName());
            }
        }
    }
}
