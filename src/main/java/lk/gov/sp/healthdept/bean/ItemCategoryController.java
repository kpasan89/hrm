...
import lk.gov.sp.healthdept.facade.ItemCategoryFacade;
import lk.gov.sp.healthdept.entity.ItemCategory;
...
@ManagedBean
@SessionScoped
public final class ItemCategoryController  implements Serializable {

    @EJB
    private ItemCategoryFacade ejbFacade;
    SessionController sessionController = new SessionController();
    List<ItemCategory> lstItems;
    private ItemCategory current;
    private DataModel<ItemCategory> items = null;
    ...
    public List<ItemCategory> getLstItems() {
        return getFacade().findBySQL("Select d From ItemCategory d");
    }

    public void setLstItems(List<ItemCategory> lstItems) {
    ...
    public ItemCategory getCurrent() {
        if (current == null) {
    ...
    public void setCurrent(ItemCategory current) {
    ...
    public DataModel<ItemCategory> getItems() {
        ...
        if (items == null) {
            if (selectText.equals("")) {
                ...
                if (items.getRowCount() > 0) {
                    items.setRowIndex(0);
                    current = (ItemCategory) items.getRowData();
    ...
    public ItemCategory searchItem(String itemName, boolean createNewIfNotPresent) {
        ItemCategory searchedItem = null;
        items = new ListDataModel(getFacade().findAll("name", itemName, true));
        if (items.getRowCount() > 0) {
            items.setRowIndex(0);
            searchedItem = (ItemCategory) items.getRowData();
        } else if (createNewIfNotPresent) {
            searchedItem = new ItemCategory();
    ...
    @FacesConverter(forClass = ItemCategory.class)
        ...
        public String getAsString(FacesContext facesContext, UIComponent component, Object object) {
            ...
            if (object instanceof ItemCategory) {
                ...
                throw new IllegalArgumentException("object " + object + " is of type "
                        + object.getClass().getName() + "; expected type: " + ItemCategoryController.class.getName());
            }
        }
    }
}
