...
import lk.gov.sp.healthdept.facade.BillFacade;
import lk.gov.sp.healthdept.facade.BillItemFacade;
import lk.gov.sp.healthdept.facade.CountryFacade;
import lk.gov.sp.healthdept.facade.CylinderFacade;
import lk.gov.sp.healthdept.facade.InstitutionFacade;
import lk.gov.sp.healthdept.facade.ItemFacade;
import lk.gov.sp.healthdept.facade.ItemUnitFacade;
import lk.gov.sp.healthdept.facade.ItemUnitHistoryFacade;
import lk.gov.sp.healthdept.facade.LocationFacade;
import lk.gov.sp.healthdept.facade.MakeFacade;
import lk.gov.sp.healthdept.facade.ManufacturerFacade;
import lk.gov.sp.healthdept.facade.ModalFacade;
import lk.gov.sp.healthdept.facade.PersonFacade;
import lk.gov.sp.healthdept.facade.SupplierFacade;
...
//        }
//    }
    public Double calculateStock(Cylinder item, Institution institution, boolean emptyUnit) {
        if (item != null && institution != null) {
            return calculateStock("SELECT SUM(i.quentity) FROM ItemUnit i WHERE i.retired=false AND i.emptyUnit = " + emptyUnit + " AND i.item.id = " + item.getId() + " AND i.institution.id = " + institution.getId());
        } else {
            return 0.0;
        }
    }
    ...
    private void saveBill() {
        ...
        if (getBill().getId() != null && getBill().getId() > 0) {
            ...
            temBill.setCreatedAt(Calendar.getInstance(TimeZone.getTimeZone("IST")).getTime());
            temBill.setCreater(getSessionController().getLoggedUser());
            getBillFacade().create(temBill);
        }
    }

    public Date getPrinterDateTime() {
    ...
    private ItemUnit getExistingItemUnit(Institution exIns, Unit exUnit, Cylinder exItem, boolean emptyUnit) {
        ItemUnit myIu;
        String sql;
...
//        sql = "Select iu from ItemUnit iu where   + strItem + " and " + strSerial + " and " + strIns + " and " + strUnit + " and " + strPer + " and " + strMake + " and  " + strLoc;
        ...
        sql = "Select iu from ItemUnit iu where iu.retired=false and " + strIns + " and " + strUnit + " and " + strItem + " and iu.emptyUnit = " + emptyUnit;
    ...
    private void settleBillItem(BillItem temEntry) {
        ...
        hxIns.setBeforeQty(calculateStock((Cylinder) newItemUnit.getItem(), newItemUnit.getInstitution(), temBillItem.isEmptyUnit()));
        ...
        hxIns.setEmptyUnit(temBillItem.isEmptyUnit());
        ...
        hxUnit.setEmptyUnit(temBillItem.isEmptyUnit());
        ...
        hxLoc.setEmptyUnit(temBillItem.isEmptyUnit());
        ...
        hxPer.setEmptyUnit(temBillItem.isEmptyUnit());
        hxPer.setToIn(Boolean.TRUE);
        hxPer.setToOut(Boolean.FALSE);

        getItemUnitFacade().edit(newItemUnit);

        hxIns.setAfterQty(calculateStock((Cylinder) newItemUnit.getItem(), newItemUnit.getInstitution(), temBillItem.isEmptyUnit()));
