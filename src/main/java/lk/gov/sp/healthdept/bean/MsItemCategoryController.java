...
import lk.gov.sp.healthdept.facade.PharmaceuticalItemCategoryFacade;
...
@ManagedBean
@SessionScoped
public final class MsItemCategoryController  implements Serializable {

    @EJB
    private PharmaceuticalItemCategoryFacade ejbFacade;
    SessionController sessionController = new SessionController();
    List<PharmaceuticalItemCategory> lstItems;
    private PharmaceuticalItemCategory current;
    private DataModel<PharmaceuticalItemCategory> items = null;
    private int selectedItemIndex;
    boolean selectControlDisable = false;
    boolean modifyControlDisable = true;
    String selectText = "";

    public MsItemCategoryController() {
    }

    public List<PharmaceuticalItemCategory> getLstItems() {
        return getFacade().findBySQL("Select d From PharmaceuticalItemCategory d");
    }

    public void setLstItems(List<PharmaceuticalItemCategory> lstItems) {
    ...
    public DataModel<PharmaceuticalItemCategory> getItems() {
        ...
        if (items == null) {
            if (selectText.equals("")) {
                ...
                if (items.getRowCount() > 0) {
                    items.setRowIndex(0);
                    current = (PharmaceuticalItemCategory) items.getRowData();
    ...
    public PharmaceuticalItemCategory searchItem(String itemName, boolean createNewIfNotPresent) {
        PharmaceuticalItemCategory searchedItem = null;
        items = new ListDataModel(getFacade().findAll("name", itemName, true));
        if (items.getRowCount() > 0) {
            items.setRowIndex(0);
            searchedItem = (PharmaceuticalItemCategory) items.getRowData();
        } else if (createNewIfNotPresent) {
            searchedItem = new PharmaceuticalItemCategory();
        ...
        public String getAsString(FacesContext facesContext, UIComponent component, Object object) {
            ...
            if (object instanceof PharmaceuticalItemCategory) {
                ...
                throw new IllegalArgumentException("object " + object + " is of type "
                        + object.getClass().getName() + "; expected type: " + MsItemCategoryController.class.getName());
            }
        }
    }
}
