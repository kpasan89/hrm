...
import lk.gov.sp.healthdept.facade.PersonContactFacade;
...
//
    ...
    public void issue(){
        if (current==null){
            JsfUtil.addErrorMessage("Please select a personContact");
            return;
        }
        if (person==null){
    ...
    public DataModel<PersonContact> getItems() {
        String temSql;
        if (unit == null) {
            return null;
        }
        if (!selectText.equals("")) {
            temSql = "select f from PersonContact f where f.retired=false and lower(f.name) like '%" + selectText.toLowerCase() + "%' and f.unit.id = " + unit.getId() + " order by f.name";
        } else if (!selectName.equals("")) {
            temSql = "select f from PersonContact f where f.retired=false  and lower(f.description) like '%" + selectName.toLowerCase() + "%' and f.unit.id = " + unit.getId() + " order by f.name";
        } else {
            temSql = "select f from PersonContact f where f.retired=false and f.unit.id = " + unit.getId() + " order by f.name";
        }
        items = new ListDataModel(getFacade().findBySQL(temSql));
        return items;
    }

    public DataModel<PersonContact> getLocItems() {
        String temSql;
        if (location == null) {
            return null;
        }
        temSql = "select f from PersonContact f where f.retired=false and f.location.id = " + location.getId() + " order by f.name";
        items = new ListDataModel(getFacade().findBySQL(temSql));
        return items;
    }
    ...
    public DataModel<PersonContact> getItemsLoc() {
        if (location != null) {
            return new ListDataModel<PersonContact>(getFacade().findBySQL("SELECT i From PersonContact i WHERE i.retired=false AND i.location.id = " + location.getId()));
        } else {
            return null;
        }
    }
    ...
    public DataModel<PersonContact> getItemsUni() {
        if (unit != null) {
            return new ListDataModel<PersonContact>(getFacade().findBySQL("SELECT i From PersonContact i WHERE i.retired=false AND i.unit.id = " + unit.getId()));
        } else {
            return null;
        }
    }

    public void setItemsUni(DataModel<PersonContact> itemsUni) {
