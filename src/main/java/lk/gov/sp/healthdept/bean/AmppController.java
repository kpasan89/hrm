...
import lk.gov.sp.healthdept.entity.Ampp;
...
public final class AmppController  implements Serializable {
    ...
    private DataModel<Ampp> items = null;
    ...
    public List<Ampp> getLstItems() {
        return getFacade().findBySQL("Select d From Ampp d");
    }

    public void setLstItems(List<Ampp> lstItems) {
    ...
    public DataModel<Ampp> getItems() {
    ...
    public Ampp searchItem(String itemName, boolean createNewIfNotPresent) {
        Ampp searchedItem = null;
        items = new ListDataModel(getFacade().findAll("name", itemName, true));
        if (items.getRowCount() > 0) {
            items.setRowIndex(0);
            searchedItem = (Ampp) items.getRowData();
        } else if (createNewIfNotPresent) {
            searchedItem = new Ampp();
